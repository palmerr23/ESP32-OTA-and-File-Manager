...
String processor(const String& var) {
  ...
  if (var == "FREEFILESYS") {
    tmp = "-2";
    return tmp;
    //return humanReadableSize((FILESYS.totalBytes() - FILESYS.usedBytes()));
  }

  if (var == "USEDFILESYS") {
    tmp = "-1";
    return tmp;
    //return humanReadableSize(FILESYS.usedBytes());
  }

  if (var == "TOTALFILESYS") {
    tmp = "-3";
    return tmp;
   // return humanReadableSize(FILESYS.totalBytes());
  }
  return String();
}
...
void configureWebServer() {
  ...
  server->on("/", HTTP_GET, [](AsyncWebServerRequest * request) {
    ...
    if (checkUserWebAuth(request)) {
      ...
      Serial.println(logmessage);
      request->send_P(200, "text/html", dummy_html);
    } else {
